...
public class JwtFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
		...
		final String authorization = request.getHeader(
				HttpHeaders.AUTHORIZATION); //authorization에 있는 정보 꺼내오기
		log.debug("authorization: {}", authorization);

		//authorization가 없거나 authorization의 형식이 안맞을 때
		if (authorization == null || !authorization.startsWith("Bearer ")) {
			request.setAttribute("invalidTokenException",
					ErrorCode.INVALID_TOKEN); // customauthenticationentrypoint까지 넘어가서 에러코드반환하기
